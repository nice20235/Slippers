...
class Settings(BaseSettings):
    ...
    SESSION_MAX_DAYS: int = (
        1  # Absolute maximum session lifetime (sliding disabled past this). 0 disables hard cap.
    )
    SESSION_MAX_HOURS: int = (
        8  # Alternative to DAYS. If >0, hours takes precedence. Reduced from 12 to 8.
    )
    ALLOWED_ORIGINS: str = (
        "http://localhost:3000,"
        "http://127.0.0.1:3000,"
    ...
    # Order guards
    ORDER_MAX_QTY_PER_ITEM: int = (
        500  # hard upper bound per order line to prevent accidental huge quantities
    )
