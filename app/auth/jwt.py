...
)  # Reduced from 7 to 1 day
SESSION_MAX_DAYS = int(
    os.getenv("SESSION_MAX_DAYS", "1")
)  # Default 1 day instead of 0 (disabled)
SESSION_MAX_HOURS = int(
    os.getenv("SESSION_MAX_HOURS", "8")
)  # Read from env, default 8 hours


def _calc_session_exp(
    now: datetime, existing_session_exp: datetime | None = None
) -> datetime | None:
