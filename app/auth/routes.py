...
from app.schemas.user import UserCreate, UserLogin, UserResponse, ForgotPasswordRequest
...
async def register_user(
    user_data: UserCreate, db: AsyncSession = Depends(get_db), response: Response = None
):
    ...
    access_token = create_access_token(
        data={"sub": str(user.id)}, session_exp=now_session_exp
    )
...
async def refresh_token(
    request: Request, db: AsyncSession = Depends(get_db), response: Response = None
):
    """
    ...
    access_token = create_access_token(
        data={"sub": str(user.id)}, session_exp=sess_exp_dt
    )
    refresh_token = create_refresh_token(
        data={"sub": str(user.id)}, session_exp=sess_exp_dt
    )
...
async def logout(request: Request, response: Response):
    ...
    try:
        ...
        header_auth = request.headers.get("Authorization") or request.headers.get(
            "authorization"
        )
        if header_auth:
...
async def forgot_password(
    forgot_data: ForgotPasswordRequest, db: AsyncSession = Depends(get_db)
):
    """
    ...
    updated_user = await update_user_password(
        db, forgot_data.name, forgot_data.new_password
    )
    if not updated_user:
        raise HTTPException(
