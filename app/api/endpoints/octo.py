...
class OctoCreateIn(BaseModel):
    ...
    items: Optional[list[dict]] = Field(
        None, description="Items to create order from if order_id not provided"
    )
...
class OctoCreateOut(BaseModel):
    order_id: int
    redirect_url: str
    payment_uuid: Optional[str] = Field(
        None, description="Payment UUID from bank (external gateway)"
    )
    ...
    responses={
        ...
        400: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "detail": "Merchant status [emailok] is not allowed for non-test Payments."
                    }
                }
            },
        },
...
async def create_octo_payment(
    ...
    if not oid:
        ...
        created = await create_order(
            db, internal_order, idempotency_key=None, merge_fallback=False
        )
    ...
    if order.user_id != user.id:
        raise HTTPException(
            status_code=403, detail="Order does not belong to current user"
        )
    ...
    if order.status == OrderStatus.REFUNDED:
        raise HTTPException(
            status_code=400, detail="Cannot create payment for refunded order"
        )
    ...
    if order.payment_uuid:
        ...
        logger.info(
            "Order %s already has payment_uuid %s", order.id, existing_payment_uuid
        )
    # Allow optional override if provided (defers to order.total_amount otherwise)
    override_amount = body.amount or body.total_sum
    amount = (
        int(round(order.total_amount)) if not override_amount else int(override_amount)
    )
    if amount <= 0:
        raise HTTPException(
            status_code=400, detail="Order total is zero; cannot create payment"
        )
    ...
    if not payment_uuid and res.raw:
        ...
        payment_uuid = data_section.get("octo_payment_UUID") or data_section.get(
            "payment_uuid"
        )
    if not payment_uuid:
    ...
    return OctoCreateOut(
        order_id=order.id, redirect_url=res.octo_pay_url, payment_uuid=payment_uuid
    )


@router.post(
    "/refund",
    response_model=OctoRefundOut,
    summary="Refund Octo Payment",
    responses={
        200: {
            "description": "Возврат инициирован",
            "content": {
                "application/json": {
                    "example": {"success": True, "message": "Refund initiated"}
                }
            },
        },
        400: {
            ...
            "content": {
                "application/json": {
                    "example": {"detail": "Minimum refund is >= 13000 UZS (1 USD)"}
                }
            },
        },
    },
)
async def refund_octo_payment(
    body: OctoRefundIn,
    ...
    if not order.payment_uuid:
        raise HTTPException(
            status_code=400, detail="Order has no payment_uuid; cannot refund"
        )
    # Mock external refund call
    res = await refundPayment(order.payment_uuid, int(round(order.total_amount)))
    if not res.success:
        raise HTTPException(
            status_code=400, detail=res.errMessage or "OCTO refund error"
        )
...
async def octo_notify(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not payment:
        logger.warning(
            "Payment record not found for notify: shop_tx=%s uuid=%s",
            shop_tx,
            payment_uuid,
        )
        return {"ok": True}
    ...
    if getattr(payment, "order_id", None) is None:
        try:
            ...
            if maybe_order_id is not None:
                ...
                logger.info(
                    "Linked payment %s to order_id=%s from notify payload",
                    payment.id,
                    payment.order_id,
                )
        except Exception as e:
    ...
    if getattr(updated, "order_id", None):
        if mapped == PaymentStatus.PAID:
            try:
                ...
                logger.info(
                    "Order %s set to PAID due to successful payment %s",
                    updated.order_id,
                    updated.id,
                )
                ...
                try:
                    ...
                    paid_order = await _get_order(
                        db, updated.order_id, load_relationships=False
                    )
                    if paid_order:
                ...
                logger.warning(
                    "Failed to update order %s status after payment: %s",
                    updated.order_id,
                    e,
                )
            ...
            try:
                ...
                logger.info(
                    "Order %s set to REFUNDED due to refund of payment %s",
                    updated.order_id,
                    updated.id,
                )
            except Exception as e:
                logger.warning(
                    "Failed to update order %s status after refund: %s",
                    updated.order_id,
                    e,
                )
    else:
