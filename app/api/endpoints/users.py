...
async def get_own_profile(current_user=Depends(get_current_user)):
    """
...
async def get_user_detail(
    ...
    logger.info(
        f"Admin {admin.name} viewing user details: {user.name} ({user.phone_number})"
    )
...
async def update_user_endpoint(
    ...
    if user.id == admin.id:
        logger.warning(
            f"Admin {admin.name} attempted to update themselves through admin endpoint"
        )
        raise HTTPException(
            status_code=400,
    ...
    try:
        ...
        if (
            "phone_number" in payload_admin
            and not (payload_admin.get("phone_number") or "").strip()
        ):
...
async def update_own_profile(
    ...
    if (
        ...
        and user_update.is_admin != current_user.is_admin
    ):
        raise HTTPException(
            status_code=403, detail="You cannot change your admin status"
        )
    ...
    if user_update.new_password is not None:
        ...
        if not verify_password(
            user_update.current_password or "", current_user.password_hash
        ):
