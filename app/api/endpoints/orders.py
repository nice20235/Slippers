...
async def create_order_from_cart(
    ...
    clear_cart: bool = Query(
        False, description="If true, clears cart after creating the order"
    ),
):
...
async def list_orders(
    ...
    try:
        ...
        if finance and finance.lower() == "paid_refunded":
            ...
            if user.is_admin:
                logger.info(
                    "Admin %s listing orders with finance filter paid_refunded",
                    user.name,
                )
                rows, total = await get_orders_by_payment_statuses(
                    db, statuses=statuses, load_relationships=False
                )
            else:
                logger.info(
                    "User %s listing OWN orders with finance filter paid_refunded",
                    user.name,
                )
                rows, total = await get_orders_by_payment_statuses(
                    db, statuses=statuses, user_id=user.id, load_relationships=False
                )
            ...
            if user_ids:
                ...
                user_rows = await db.execute(
                    select(_User.id, _User.name, _User.surname).where(
                        _User.id.in_(user_ids)
                    )
                )
            ...
            if user.is_admin:
                ...
                orders, total = await get_orders(
                    db, skip=0, limit=100000, load_relationships=False
                )
            else:
                logger.info(f"User {user.name} listing OWN orders")
                orders, total = await get_orders(
                    db, skip=0, limit=100000, user_id=user.id, load_relationships=False
                )
            # Batch load items for all orders
            ...
            return [
                {
                    ...
                    "user_id": order.user_id,
                    "user_name": (
                        f"{users_by_id[order.user_id][0]} {users_by_id[order.user_id][1]}".strip()
...
async def get_order_endpoint(
    order_id: int, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)
):
    ...
    try:
        ...
        if not user.is_admin and db_order.user_id != user.id:
            logger.warning(
                f"User {user.name} attempted to access order {order_id} belonging to user {db_order.user_id}"
            )
            raise HTTPException(
...
async def update_order_endpoint(
    ...
    if not user.is_admin and db_order.user_id != user.id:
        logger.warning(
            f"User {user.name} (Admin: {user.is_admin}) attempted to update order {order_id} belonging to user {db_order.user_id}"
        )
        raise HTTPException(
            status_code=403, detail="You can only update your own orders"
        )

    logger.info(
        f"Updating order {order_id} by user: {user.name} (Admin: {user.is_admin})"
    )
...
async def delete_order_endpoint(
    ...
    if not user.is_admin and db_order.user_id != user.id:
        logger.warning(
            f"User {user.name} (Admin: {user.is_admin}) attempted to delete order {order_id} belonging to user {db_order.user_id}"
        )
        raise HTTPException(
            status_code=403, detail="You can only delete your own orders"
        )

    logger.info(
        f"Deleting order {order_id} by user: {user.name} (Admin: {user.is_admin})"
    )
