...
async def get_user(
    db: AsyncSession, user_id: int, load_orders: bool = False
) -> Optional[User]:
...
async def get_user_by_name(
    db: AsyncSession, name: str, load_orders: bool = False
) -> Optional[User]:
...
async def update_user(
    db: AsyncSession, db_user: User, user_update: UserUpdate, load_orders: bool = False
) -> User:
    ...
    if load_orders:
        result = await db.execute(
            select(User).options(selectinload(User.orders)).where(User.id == db_user.id)
        )
...
async def update_user_password(
    db: AsyncSession, name: str, new_password: str, load_orders: bool = False
) -> Optional[User]:
