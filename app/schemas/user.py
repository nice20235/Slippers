...
class UserBase(BaseModel):
    ...
    surname: str = Field(
        ..., description="User's last name", min_length=2, max_length=100, example="Doe"
    )
    ...
    is_admin: bool = Field(
        default=False, description="Whether user is an admin", example=False
    )


class UserCreate(UserBase):
    password: str = Field(
        ..., description="User's password", min_length=8, example="securepassword123"
    )
    confirm_password: str = Field(
        ..., description="Confirm password", example="securepassword123"
    )
...
class UserUpdate(BaseModel):
    ...
    is_admin: Optional[bool] = Field(
        None, description="Whether user is an admin", example=False
    )
...
class UserInDB(UserBase):
    ...
    created_at: datetime = Field(
        ..., description="Account creation timestamp", example="2024-01-15T10:30:00Z"
    )
    updated_at: datetime = Field(
        ..., description="Last update timestamp", example="2024-01-15T10:30:00Z"
    )
...
class UserLogin(BaseModel):
    name: str = Field(..., description="User's first name", example="John")
    password: str = Field(
        ..., description="User's password", example="securepassword123"
    )

    class Config:
        json_schema_extra = {
            "example": {"name": "John", "password": "securepassword123"}
        }
...
class RefreshTokenRequest(BaseModel):
    refresh_token: Optional[str] = Field(
        None, description="JWT refresh token (can also be provided in headers)"
    )
...
class ForgotPasswordRequest(BaseModel):
    ...
    new_password: str = Field(
        ..., description="New password", min_length=8, example="newsecurepassword123"
    )
    confirm_new_password: str = Field(
        ..., description="Confirm new password", example="newsecurepassword123"
    )
...
class UserSelfUpdate(UserUpdate):
    ...
    confirm_new_password: Optional[str] = Field(
        None, description="Confirm new password", example="newsecurepassword123"
    )
