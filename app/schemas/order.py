...
class OrderItemBase(BaseModel):
    ...
    quantity: int = Field(..., description="Quantity ordered", gt=0, le=500, example=2)
    unit_price: float = Field(
        ...,
        description="Unit price at order time (ignored on create, taken from product)",
        gt=0,
        example=650.0,
    )
...
class OrderItemUpdate(BaseModel):
    quantity: Optional[int] = Field(
        None, description="Quantity ordered", gt=0, le=100, example=2
    )
    unit_price: Optional[float] = Field(
        None, description="Unit price", gt=0, example=650.0
    )
    notes: Optional[str] = Field(
        None,
        description="Special instructions",
...
class OrderItemInDB(OrderItemBase):
    ...
    total_price: float = Field(
        ..., description="Total price for this item", example=1300.0
    )
    created_at: datetime = Field(
        ..., description="Item creation timestamp", example="2024-01-15T10:30:00Z"
    )
...
class OrderCreatePublic(BaseModel):
    items: List[OrderItemCreatePublic] = Field(
...
class OrderBase(BaseModel):
    order_id: str = Field(
        ..., description="Unique order identifier", min_length=1, max_length=32
    )
    user_id: int = Field(..., description="User ID", gt=0)
    status: OrderStatus = Field(
        default=OrderStatus.PENDING, description="Order status (PENDING|PAID|REFUNDED)"
    )
...
class OrderStatusUpdate(BaseModel):
    ...
    notes: Optional[str] = Field(
        None, description="Status update notes", max_length=500
    )
