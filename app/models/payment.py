...
class Payment(Base):
    ...
    shop_transaction_id: Mapped[str] = mapped_column(
        String(64), unique=True, index=True
    )
    octo_payment_uuid: Mapped[str | None] = mapped_column(
        String(64), unique=False, nullable=True, index=True
    )
    ...
    )  # store compact json string if needed
